...
from keyboards.keyboards import (
    get_new_game_kb, get_ready_kb, get_guess_pass_kb)
...
async def process_ready_press(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        text=text, reply_markup=get_guess_pass_kb())
...
async def process_guess_press(callback: CallbackQuery):
    ...
    word = generate_new_word(user_id)
    curr_word: int = users_db[user_id].get_current_word()
    users_db[user_id].calculate_score(
        users_db[user_id].get_current_team(), guess=1)
    if users_db[user_id].get_time_is_over() is True:
        await callback.message.edit_text(
...
async def process_pass_press(callback: CallbackQuery):
    ...
    curr_word: int = users_db[user_id].get_current_word()
    users_db[user_id].calculate_score(
        users_db[user_id].get_current_team(), pas=1)
    if users_db[user_id].get_time_is_over() is True:
        await callback.message.edit_text(
            callback.message.text + ' ❌\n\n⌛️ <b>Время вышло!</b>')
        return None
...
def register_game_handlers(dp: Dispatcher):
    dp.register_callback_query_handler(
        process_ready_press, text='ready_button_pressed')
    dp.register_callback_query_handler(
        process_guess_press, text='guess_button_pressed')
    dp.register_callback_query_handler(
        process_pass_press, text='pass_button_pressed')
